...
const MessageInput = ({ onSendMessage, onStopGeneration, onClearChat, onRegenerate, isLoading, activeChannel, canRegenerate, draft, onDraftChange, isLimitReached }) => {
  ...
  const getPlaceholder = () => {
      if (isLimitReached) return "Daily request limit reached. Increase it in Settings to continue.";
        ...
        <div className={`relative bg-white dark:bg-zinc-900 rounded-3xl border transition-all duration-300 overflow-hidden flex flex-col ${isFocused ? 'border-indigo-200 ring-4 ring-indigo-100 dark:ring-1 dark:ring-indigo-500/20 dark:border-indigo-500/30 shadow-xl shadow-indigo-100 dark:shadow-none' : 'border-zinc-200 dark:border-zinc-800 shadow-2xl shadow-zinc-200/50 dark:shadow-none'}`}>
            ...
            <div className="px-4 pt-3 pb-2">
                <textarea
                    ...
                    className={`w-full bg-transparent text-[15px] text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-600 outline-none resize-none max-h-40 scrollbar-thin leading-relaxed text-center ${isLimitReached ? 'cursor-not-allowed opacity-50' : ''}`}
                />
            </div>
            ...
            <div className="px-2 pb-2 flex items-center justify-between">
                <div className="flex items-center gap-0.5 px-2" role="toolbar" aria-label="Formatting options">
                    ...
                    <input 
                        ...
                        disabled={isLimitReached}
                    />
                    <ToolbarButton onClick={() => fileInputRef.current?.click()} disabled={isLoading || isLimitReached} ariaLabel="Attach Image" title="Attach Image">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>
                    </ToolbarButton>

                    <div className="w-px h-4 bg-zinc-200 dark:bg-zinc-800 mx-1"></div>

                    <ToolbarButton onClick={() => onDraftChange(text + '**bold**')} disabled={isLoading || isLimitReached} ariaLabel="Bold" title="Bold">
                    ...
                    <ToolbarButton onClick={() => onDraftChange(text + '- item')} disabled={isLoading || isLimitReached} ariaLabel="Bulleted List" title="List">
                    ...
                    {canRegenerate && (
                         <ToolbarButton onClick={onRegenerate} disabled={isLoading || isLimitReached} ariaLabel="Regenerate last response" title="Regenerate">
                ...
                <div className="flex items-center gap-3">
                     <span className="hidden sm:inline-block text-[10px] text-zinc-300 dark:text-zinc-600 font-medium" aria-hidden="true">
                         {isLimitReached ? 'Limit Reached' : (text.length > 0 || attachments.length > 0) ? 'Press Enter' : 'Ready'}
                     </span>
                    {isLoading ? (
                        <button 
                        ...
                        <button 
                            onClick={handleSend}
                            disabled={(!text.trim() && attachments.length === 0) || isLimitReached}
                            aria-label="Send message"
                            className={`w-8 h-8 rounded-full flex items-center justify-center transition-all shadow-md ${((text.trim() || attachments.length > 0) && !isLimitReached) ? 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-200' : 'bg-zinc-100 dark:bg-white/5 text-zinc-300 dark:text-zinc-600'}`}
                        >
