...
const MessageInput = ({ onSendMessage, onStopGeneration, onClearChat, onRegenerate, isLoading, activeChannel, canRegenerate, draft, onDraftChange }) => {
  ...
  // Handle text changes via parent prop
  const setText = (val) => onDraftChange(val);
  const text = draft;
  ...
  const ToolbarButton = (props) => (
    <button
        type="button"
        className={`w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 ${
            props.disabled 
    ...
    <div className={expanded ? "flex flex-col h-full" : ""}>
        <div className="flex items-center px-3 pt-3 gap-1 border-b border-transparent focus-within:border-zinc-200/50 dark:focus-within:border-white/10 transition-colors" role="toolbar" aria-label="Message formatting options">
             <ToolbarButton onClick={() => insertFormat('**', '**')} disabled={isLoading} ariaLabel="Bold" title="Bold">
                 <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M6 4h8a4 4 0 014 4 4 4 0 01-4 4H6V4zm0 8h9a4 4 0 014 4 4 4 0 01-4 4H6v-8z" /></svg>
             </ToolbarButton>
             <ToolbarButton onClick={() => insertFormat('*', '*')} disabled={isLoading} ariaLabel="Italic" title="Italic">
                 <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M10 20l4-16m4 4l4 4M6 16l-4-4" /></svg>
             </ToolbarButton>
             ...
             <ToolbarButton onClick={() => setIsExpanded(!isExpanded)} disabled={isLoading} ariaLabel={isExpanded ? "Minimize" : "Maximize"} title={isExpanded ? "Minimize" : "Maximize"} className="hidden md:flex">
                 {isExpanded ? (
             ...
             <div className="flex-1 flex items-center justify-end px-2">
                <div className="w-px h-4 bg-zinc-300/50 dark:bg-zinc-700/50"></div>
             </div>
          ...
          <div className="px-4 py-3 flex-1">
            <textarea
              ...
              rows={1}
              className={`w-full bg-transparent text-zinc-900 dark:text-zinc-100 placeholder-zinc-500/60 dark:placeholder-zinc-400/60 text-base leading-relaxed outline-none resize-none py-1 scrollbar-thin ${expanded ? 'h-full' : 'min-h-[48px]'}`}
              style={expanded ? {} : { maxHeight: '240px' }}
            />
          </div>
