...
const Message = ({ message, ...props }) => {
  ...
  const CopyButton = ({ onClick, type, title }) => (
      <button
        onClick={onClick}
        title={title}
        className="h-9 w-9 flex items-center justify-center bg-white/60 dark:bg-white/5 rounded-lg border border-white/40 dark:border-white/10 text-zinc-500 dark:text-zinc-400 hover:bg-white hover:text-zinc-900 dark:hover:text-white shadow-sm transition-all backdrop-blur-sm"
      >
         {copiedType === type ? (
